...
export default function SimpleCompatibility() {
  ...
  const handleSubmit = (event) => {
    ...
    // 성격 차이 설명
    let personalityDifferences = `${name1}님은 ${compatibilityResult.personality.person1} `;
    personalityDifferences += `${name2}님은 ${compatibilityResult.personality.person2}`;

    // 궁합에 대한 설명
    let harmony = `${name1}님과 ${name2}님은 서로 성격 차이가 있지만, `;
    harmony += `서로를 배려하는 성향 덕분에 조화를 이룰 수 있습니다. 서로의 차이를 이해하고 존중한다면 좋은 관계를 유지할 수 있습니다.`;

    setResult({
      compatibility: compatibilityResult.compatibility,
      personality: compatibilityResult.personality,
  ...
  return (
    <div className="container">
      ...
      <div className="home-link">
        <Link href="/" className="back-link">← 메인 페이지로 돌아가기</Link>
      </div>
