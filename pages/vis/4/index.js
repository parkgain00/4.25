...
export default function Vis4() {
  ...
  useEffect(() => {
    ...
    let handImg, maskedHand;

    function sketch(p) {
      let path = [];
      let totalPoints = 1400;
      let tangledStart = 400;
      let tangledEnd = 1000;
      let baseStep = 6;
      let drawnPoints = 0;
      ...
      function generateTanglePath() {
        ...
        for (let i = 1; i < totalPoints; i++) {
          let step, da, dr;
          if (i < tangledStart) {
            step = baseStep + p.noise(i * 0.01) * 2;
            da = p.noise(i * 0.02) * 0.7 - 0.35 + p.random(-0.1, 0.1);
            dr = p.noise(i * 0.03) * 8 - 4;
          } else if (i < tangledEnd) {
            step = baseStep * (0.7 + p.noise(i * 0.1) * 1.2);
            da = p.noise(i * 0.2) * 2.5 - 1.25 + p.random(-0.7, 0.7);
            dr = p.noise(i * 0.15) * 60 - 30 + p.random(-30, 30);
          } else {
            step = baseStep + p.noise(i * 0.01 + 100) * 2;
            da = p.noise(i * 0.02 + 100) * 0.7 - 0.35 + p.random(-0.1, 0.1);
            dr = p.noise(i * 0.03 + 100) * 8 - 4;
          }
          ...
          prevAngle += da;
          r += dr * 0.05;
          r = p.constrain(r, Math.min(w, h) / 10, Math.min(w, h) / 2 - 20);
          x += Math.cos(prevAngle) * step;
          y += Math.sin(prevAngle) * step;
          x = p.constrain(x, 2, w - 2);
          y = p.constrain(y, 2, h - 2);
          tanglePath.push({ x, y });
        }
      }
      ...
      p.draw = function () {
        ...
        if (maskedHand) {
          // 왼쪽 타원 (그대로)
          p.image(maskedHand, leftX - ellipseW/2, yMid - ellipseH/2);
          // 오른쪽 타원 (좌우 반전)
          p.push();
          p.translate(rightX, yMid);
          p.scale(-1, 1);
          p.image(maskedHand, -ellipseW/2, -ellipseH/2);
          p.pop();
        }
        p.pop();
